...
> **注意**: 当前 GitHub Actions 构建的产物为 **绿色便携版 (Portable Build)**，无需安装，解压即用。

### 1. 启动核心服务
1. 打开解压后的 **`LaiCai-Windows-x64`** 文件夹。
2. 进入 **`laiCai`** 子文件夹（即截图中第三个文件夹）。
3. 双击运行 **`laiCai.exe`**。
   - ⚠️ **安全提示**: 首次运行时 Windows SmartScreen 可能会拦截，请点击 **“更多信息”** -> **“仍要运行”**。
4. 此时会弹出一个黑色命令行窗口，显示系统正在初始化，**请保持该窗口运行**。

### 2. 访问系统
服务启动后，打开浏览器访问：
- **系统控制台 / API**: http://localhost:8000

*(注：后续版本将提供 `Setup.exe` 一键安装包及桌面快捷方式。)*

...
def get_ths_global(var_name):
    ...
    if IN_XIADAN_API:
        try:
            val = getattr(xd, var_name, getattr(xd, "g_{}".format(var_name), None))
            if val is not None:
                return val
        except Exception:
            pass
...
def _normalize_status(raw_status, status_text, filled_qty, qty, cancel_qty):
    text = "{} {}".format(raw_status or "", status_text or "").strip().lower()
...
def _iter_order_candidates(raw_orders):
    rows = []
...
def _normalize_orders(raw_orders, source):
    rows = []
...
def _merge_orders(*order_groups):
    merged = {}
    for rows in order_groups:
        for row in rows:
            key = str(row.get("order_id") or row.get("client_order_id") or "").strip()
            if not key:
                key = "tmp-{}-{}".format(row.get("ticker", ""), int(_to_float(row.get("timestamp")) * 1000))
...
def _build_orders_payload(mode, include_local_store=True):
...
def process_request(req_data):
    try:
        ...
        if action in {"buy", "sell"}:
            ...
            client_order_id = str(req_data.get("client_order_id") or "ths-{}".format(int(time.time() * 1000)))
            cmd_str = "{} {} {} {} -notip".format(action, ticker, price, qty)
        ...
        if action == "cancel":
            ...
            if htbh:
                cmd_str = "cancel -h {}".format(htbh)
            else:
                ticker = str(req_data.get("ticker") or "").strip()
                direction = str(req_data.get("direction") or "").strip()
                cmd_str = "cancel {} {}".format(ticker, direction).strip()
            res = execute_cmd(cmd_str)
            retcode = str(res.get("retcode", "0")).strip()
            ok = retcode in {"", "0"}
...
def run_server():
    ...
    print("[来财] 监听端口: {}:{}".format(HOST, PORT))
    print("=====================================")

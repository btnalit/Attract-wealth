...
import { 
  Search, ChevronDown, TrendingUp, TrendingDown, BarChart2, Settings,
  ArrowUpRight, ArrowDownLeft, Share2, LineChart, Activity, Zap, Clock, Loader2, RefreshCcw
} from 'lucide-react';
      ...
      <div className="flex items-center justify-between border-b border-border bg-bg-card/50 p-2 px-4 backdrop-blur-sm">
        <form onSubmit={handleSearch} className="flex items-center gap-6">
          <div className="flex h-8 items-center rounded-sm border border-border bg-bg-primary/50 px-3 transition-colors hover:border-neon-cyan/50 focus-within:border-neon-cyan/80">
            ...
            <input 
              ...
              placeholder="SEARCH SYMBOL (e.g. sh600000)" 
          ...
          <div className="flex items-center gap-4">
            <div className="flex flex-col">
              <span className="text-[10px] font-bold text-white uppercase tracking-wider">{quote?.name || ticker} / CN STOCK</span>
              <span className="text-[9px] text-info-gray/60">REALTIME DATA {loading && <Loader2 className="inline h-2 w-2 animate-spin ml-1" />}</span>
            </div>
            ...
            ) : <span className="text-[10px] text-info-gray">NO DATA</span>}
          </div>
        </form>
        ...
        <div className="flex items-center gap-3">
          <div className="flex h-8 items-center gap-1.5 rounded-sm border border-border px-3 bg-bg-primary/30">
            ...
            <span className="text-[9px] font-bold uppercase text-info-gray">SOURCE: AKSHARE</span>
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="flex flex-[3] flex-col border-r border-border">
          ...
          <div className="relative flex-1 bg-bg-primary overflow-hidden">
            <div className="absolute left-4 top-4 z-10 flex gap-2">
              <Badge variant="outline" className="bg-bg-card/80 backdrop-blur-md">VOL: {quote?.volume || '--'}</Badge>
              <Badge variant="outline" className="bg-bg-card/80 backdrop-blur-md">AMT: {quote?.turnover || '--'}</Badge>
            </div>
            
            <div className="flex h-full w-full items-center justify-center">
              <div className="flex flex-col items-center gap-4 text-info-gray/20">
                <LineChart className="h-24 w-24 stroke-[1px]" />
                <span className="font-orbitron text-sm font-bold uppercase tracking-[0.4em]">TradingView Charting Interface</span>
                <span className="text-[10px] font-mono">[ CONNECTED TO {ticker.toUpperCase()} ]</span>
              </div>
            </div>
          ...
          <div className="flex h-[240px] flex-col border-t border-border bg-bg-card/30">
            <div className="flex h-8 items-center border-b border-border bg-bg-card/50 px-4">
              <span className="text-[10px] font-bold uppercase tracking-widest text-neon-cyan">Execution Panel</span>
            </div>
              ...
              <div className="flex flex-col gap-3">
                <div className="grid grid-cols-2 gap-2">
                  <div className="flex flex-col gap-1">
                    <label className="text-[9px] uppercase text-info-gray/60 font-bold">Limit Price</label>
                  ...
                  <div className="flex flex-col gap-1">
                    <label className="text-[9px] uppercase text-info-gray/60 font-bold">Quantity</label>
        ...
        <div className="flex flex-1 flex-col overflow-y-auto bg-bg-card/30 custom-scrollbar">
          <div className="p-4 border-b border-border">
            <h3 className="mb-4 text-[10px] font-bold uppercase tracking-[0.2em] text-neon-cyan">Technical Pulse</h3>
            <div className="flex flex-col gap-3">
...
const Indicator: React.FC<any> = ({ label, value, percent, color = "text-white" }) => (
  <div className="flex flex-col gap-1">
...
const Button: React.FC<any> = ({ children, className, variant, ...props }) => (
  <button className={cn("px-3 py-1.5 rounded-sm text-xs font-bold transition-all border", 
    variant === 'outline' ? "border-border text-info-gray hover:text-white" : 

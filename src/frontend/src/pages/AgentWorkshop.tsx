...
const initialNodes: Node[] = [
  // Input Node
  { id: 'DataCollector', type: 'agent', data: { label: 'Data Collector', id: 'DataCollector' }, position: { x: 300, y: 0 } },
  
  // Analysts
  { id: 'FundamentalAnalyst', type: 'agent', data: { label: 'Fundamental Analyst', id: 'FundamentalAnalyst' }, position: { x: 0, y: 150 } },
  { id: 'TechnicalAnalyst', type: 'agent', data: { label: 'Technical Analyst', id: 'TechnicalAnalyst' }, position: { x: 200, y: 150 } },
  { id: 'NewsAnalyst', type: 'agent', data: { label: 'News Analyst', id: 'NewsAnalyst' }, position: { x: 400, y: 150 } },
  { id: 'SentimentAnalyst', type: 'agent', data: { label: 'Sentiment Analyst', id: 'SentimentAnalyst' }, position: { x: 600, y: 150 } },
  
  // Debate & Researcher
  { id: 'DebateResearcher', type: 'agent', data: { label: 'Debate Researcher', id: 'DebateResearcher' }, position: { x: 300, y: 300 } },
  
  // Decision
  { id: 'TraderDecision', type: 'agent', data: { label: 'Trader Decision', id: 'TraderDecision' }, position: { x: 300, y: 450 } },
  
  // Risk
  { id: 'RiskGuard', type: 'agent', data: { label: 'Risk Guard', id: 'RiskGuard' }, position: { x: 300, y: 600 } },
  
  // Output
  { id: 'ExecutionOutput', type: 'agent', data: { label: 'Execution Output', id: 'ExecutionOutput' }, position: { x: 300, y: 750 } },
];
    ...
    <div className="flex h-[calc(100vh-80px)] w-full overflow-hidden bg-bg-primary">
      ...
      <div className="relative flex-1 h-full border-r border-border/50 shadow-[inset_0_0_50px_rgba(0,0,0,0.5)]">
        <div className="absolute top-4 left-6 z-10 flex items-center gap-2 px-3 py-1.5 rounded-full bg-bg-card/80 border border-border backdrop-blur-md">
          ...
          <span className="text-xs font-mono font-bold tracking-[0.2em] text-neon-cyan">LANGGRAPH ENGINE v1.2</span>
          ...
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-[10px] text-info-gray uppercase tracking-wider">Stream Active</span>
          </div>
        </div>
        ...
        >
          ...
          <Panel position="top-right" className="bg-bg-card/90 border border-border p-2 rounded-lg backdrop-blur-md flex flex-col gap-2">
            ...
            >
              ...
              <span className="text-xs font-bold uppercase tracking-widest font-mono">Run Workflow</span>
            </button>
            <button 
              onClick={() => resetGraph()}
            ...
            >
              ...
              <span className="text-xs font-bold uppercase tracking-widest font-mono">Reset Graph</span>
            </button>
          </Panel>
          
          <Panel position="bottom-left" className="bg-bg-card/90 border border-border p-4 rounded-lg backdrop-blur-md">
            <div className="flex flex-col gap-1">
              <span className="text-[10px] text-info-gray uppercase font-mono">Session Performance</span>
              <div className={clsx(
      ...
      <aside className="w-[400px] flex flex-col bg-bg-card border-l border-border">
        <header className="p-4 border-b border-border flex items-center justify-between bg-bg-primary/50">
          <div className="flex items-center gap-2 text-neon-cyan">
            ...
            <h2 className="text-xs font-bold uppercase tracking-[0.2em] font-mono">Inspector</h2>
          </div>
          <div className="px-2 py-0.5 rounded text-[10px] bg-bg-primary text-info-gray font-mono">
            {selectedNodeId || activeNodeId || 'No Selection'}
          </div>
        </header>
          ...
          <div className="space-y-3">
            ...
            {selectedNodeLogs.length > 0 ? (
              ...
              <div className="h-40 flex flex-col items-center justify-center border-2 border-dashed border-border rounded-xl">
                ...
                <p className="text-[10px] text-info-gray font-mono uppercase">Waiting for logs...</p>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="p-4 border-t border-border bg-bg-primary/50 h-[180px]">
          <h3 className="text-[10px] font-bold text-info-gray uppercase tracking-widest mb-3">Resource Monitors</h3>
          <div className="grid grid-cols-2 gap-3">
            <div className="p-3 bg-bg-card border border-border rounded">
              <span className="text-[9px] text-info-gray uppercase font-mono block mb-1">Response Time</span>
              <span className="text-sm font-bold text-white font-mono">1.2s</span>
            </div>
            <div className="p-3 bg-bg-card border border-border rounded">
              <span className="text-[9px] text-info-gray uppercase font-mono block mb-1">Tokens Used</span>
              <span className="text-sm font-bold text-white font-mono">4.2k</span>
            </div>
            <div className="p-3 bg-bg-card border border-border rounded">
              <span className="text-[9px] text-info-gray uppercase font-mono block mb-1">Confidence</span>
              <span className="text-sm font-bold text-up-green font-mono">92%</span>
            </div>
            <div className="p-3 bg-bg-card border border-border rounded">
              <span className="text-[9px] text-info-gray uppercase font-mono block mb-1">Retries</span>
